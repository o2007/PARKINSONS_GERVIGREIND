...
axes[0].set_xlabel("Töluleg skekkja", fontsize=12)
axes[0].set_ylabel("Fjöldi", fontsize=12)
axes[0].set_title("Dreifing tölulegrar skekkju", fontsize=13)
axes[0].legend()
...
axes[1].annotate(f"{pct_within_10:.1f}% innan 10 pts",
                 ...
                 arrowprops=dict(arrowstyle="->"), fontsize=9)
axes[1].set_xlabel("Þröskuldur", fontsize=12)
axes[1].set_ylabel("% spáa", fontsize=12)
axes[1].set_title("Uppsöfnuð skekkjudreifing", fontsize=13)
axes[1].legend()
...
axes[1].set_xlabel("Raunverulegt UPDRS-III", fontsize=12)
axes[1].set_ylabel("Spáð UPDRS-III", fontsize=12)
...
for i, patno in enumerate(selected_pats):
    ...
    ax.plot(all_t, all_s, marker="o", linewidth=2.5,
            color="steelblue", label="Mæld gildi", zorder=3)
    ax.plot(target_years, predicted_future, marker="s", linestyle="--",
            linewidth=2, color="coral", label="Spá", zorder=3)
...
fig.suptitle(
    "Prófun á fimm sjúklingum — Mæld gildi vs Spáð framtíðar UPDRS-III gildi\n"
...
ax.plot(years, act_mean,  marker="o", linewidth=2.5, markersize=7,
        color="steelblue", label="Mæld gildi (meðaltal)")
ax.plot(years, pred_mean, marker="s", linewidth=2.5, markersize=7,
        linestyle="--", color="coral", label="Spá (meðaltal)")
